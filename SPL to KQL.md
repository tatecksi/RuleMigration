...
<sub>stats | <sub>Provides statistics, grouped optionally by fields. | | <sub>[summarize](https://docs.microsoft.com/azure/data-explorer/kusto/query/summarizeoperator) | <sub><pre> Sales \| summarize NumTransactions=count(), Total=sum(UnitPrice * NumUnits) by Fruit, StartOfMonth=startofmonth(SellDateTime) </pre>
